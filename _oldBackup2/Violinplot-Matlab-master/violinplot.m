...
elseif isnumeric(data) % numeric input   
    ...
    if hascategories && numel(data) == numel(cats)    
        ...
        if strcmp(violins(1).Orientation,'vertical')
% % % below modified by wp			  
% % %             set(violins(1).Parent, 'XTick', 1:length(catnames), 'XTickLabels', catnames);
            set(violins(1).Parent, 'XTick', 1:length(catnames), 'XTickLabels', catnames_labels);
        else
% % %             set(violins(1).Parent, 'YTick', 1:length(catnames), 'YTickLabels', catnames);
            set(violins(1).Parent, 'YTick', 1:length(catnames), 'YTickLabels', catnames_labels);
        end
% % % above modified by wp			  
        set(violins(1).Parent,'Box','on');
        return
    else
        data = {data};
