function [scan3dd] = fcv_scan3dd_to_mat(filename)
	% % % renamed for convenience of calling. All rights go to the original
	% authors
...
% function [scan3dd] = elec_load_scan_3ddasc(filename)
 ...
 while ok,
   ...
   if numel(tmp)>3 && tmp(end-3) == 32,	%%% my line to avoid error
%    if tmp(end-3) == 32,		%%% original line
     % found a head shape point
     scan3dd.hsp(end+1,:) = [tmp(end-2),tmp(end-1),tmp(end)];
     continue;
   end
