function [c, r, pp] = neu_cluster4pac_selected (conArray, neighborDefinition, conThresh, spfThresh, ...
	method, shiftPredictor, nClustersPerTest, nTestPerCluster, fname, nPools, clusterP, sumFlag, id1, id2)
...
%   17/09/2017: first draft, adopted from the previous function of coupling cluster
	...
	if max([abs(pp.n - n1), abs(n2 - 2)]) > 0.1
		fprintf('\n Data size error!\n');
		return;
	end
	if pp.shiftFlag && max(abs(size(conArray) - size(shiftPredictor))) > 0.1
		fprintf('\n Data size error!\n');
		return;
	end
	...
	%% 2. the simplified anova for shift predicotr
	data = conArray(:, :, :, :, 1) - conArray(:, :, :, :, 2);
	if pp.shiftFlag
		tShift = mat_tValue4Arrays(abs(data) - ...
			abs(shiftPredictor(:, :, :, :, 1) - shiftPredictor(:, :, :, :, 2)), 4);
		fShift = tShift > t05;
	else
		fShift = true(size(data(:, :, :, 1)));
	end

	%% 3. cluster +/-
	fprintf('\n%s =========Data computed in on %04d-%02d-%02d %02d:%02d:%02d=========\n', method, round(clock));
	t = mat_tValue4Arrays(conArray(:, :, :, :, 1) - conArray(:, :, :, :, 2), 4);
	[r.c{1}, r.nc1, r.sz1] = insFindCluster(t, tCon, neighborDefinition, pp.spfThresh, pp.method, fShift, id1, id2);
	[r.c{2}, r.nc2, r.sz2] = insFindCluster(-t, tCon, neighborDefinition, pp.spfThresh, pp.method, fShift, id1, id2);
	save(fname, 'r', 'pp', 't');
	theSZ = [r.sz1; r.sz2];
	if isempty(theSZ)
		...
		fprintf('\n%s =========Data computed out max size %8.2f on %04d-%02d-%02d %02d:%02d:%02d=========\n', method, max(theSZ), round(clock));
	end
	%% 4. statistical test
	% % % initialize
	...
	if nPools > 1
		...
		parfor ts = 1 : pp.nTestPerCluster
			...
			t1 = cmp04_Tvalue(cz, 4, 0);

			% % % do clustering
			[tmp1, tmp2, sz0] = insFindCluster(t1, tCon, neighborDefinition, pp.spfThresh, pp.method, fShift, id1, id2);
			tmp1 = [];
			tmp2 = [];
			cz = [];
			t1 = [];
...
function [c, nc, sz] = insFindCluster(t, t0, nb, th, method, ctrl, id1, id2)
	...
	switch lower(method(1))
		case 'n'
			[c, nc] = cluster4Node(bA, id1, id2);
		case 'e'
			[c, nc] = cluster4Edge(bA, nb, id1, id2);
		otherwise
...
function [c, nc] = cluster4Node(in, id1, id2)
	% % % input check
	sz =size(in); %[n n T]
	idx1 = true(sz(1), 1);
	idx1(id1) = false;
...
function [c, nc] = cluster4Edge(in, nb, id1, id2)
	sz =size(in);
	idx1 = true(sz(1), 1);
	idx1(id1) = false;
	idx2 = true(sz(2), 1);
