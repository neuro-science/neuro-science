...
F = permute(F, [1 3 4 2]); % [1 1 1 nFFT]
ramp = permute(linspace(1, 0, nFFT-nVol), [1 3 4 2]); % [1 1 1 nRamp]

nii.img = single(nii.img); % avoid integer type
for i = 1:nSL % slice by slice fft/ifft seems faster than all together
    y = nii.img(:,:,i,:); % one slice: [r c 1 nVol]
    ...
    y = fft(y, [], 4);
    y = bsxfun(@times, y, exp(t(i)*F*sqrt(-1))); % shift phase
    y = real(ifft(y, [], 4));
    nii.img(:,:,i,:) = y(:,:,1,1:nVol); % replace img while drop padding
end
