function cfg = plt08_tfPlot( data, cfg )
...
% % % plot data as 2-D color image
	...
	if nargin < 2
		...
		y = 1 : ny;
		% % % plot
		h = image(x, y, data, 'parent', cfg.aid);
		set(cfg.aid, 'ydir', 'normal');
		return
	end
	...
	% % % version check needed		
		...
		pZ = F(pX, pY);
		if isfield(cfg, 'cLim')
			cZ = plt03_scale2color(pZ(:), 0, cfg.cLim);
		else
			cZ = plt03_scale2color(pZ(:), 0);
		end
		...
		if isfield(cfg, 'cLim')
			cMap = plt03_scale2color(data(:), 0, cfg.cLim);
		else
			cMap = plt03_scale2color(data(:), 0);
		end
