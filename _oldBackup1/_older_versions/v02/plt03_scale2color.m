function [cout, sid, theMap, cMax] = plt03_scale2color(d, th, M, style)
...
% 		warning('Data input shall be Nx1 vectors!');
	...
	if nargout > 2
		switch style(1)
			case 'j'
				cMax = [-M, M]';
				theMap = plt03_scale2color(-100 : 100, th, [], 'j');
			case 'h'
				cMax = [0, M]';
				theMap = plt03_scale2color(1 : 100, th, [], 'h');
			case 'm'
				cMax = [-M, M]';
				theMap = plt03_scale2color(-100 : 100, th, [], 'm');
			otherwise
				cMax = [];
				theMap = [];
		end
