function r = meg_clusterPowerANOVA_v1(data, nb, th1, th2, nClustersPerTest, nTestPerCluster, nblimit, pth, fname, multiCoreFlag)
...
% % % written on 08/05/2017, based on the cmp30_Permute4H0_Cluster4Power
	...
	elseif isstruct(data)
		fprintf('Summary only!\n');
		doFlag = false;
		r = data;
		data = [];
		[nFs, nTs, nvxs, tmp] = size(r.F);
	else
		doFlag = true;
	...
	if nargin < 10 || isempty(multiCoreFlag)
		multiCoreFlag = true;
	end
	...
	if doFlag
		% % % the real data
		[r.F, r.p] = rmANOVA4ThisData(data, 0, multiCoreFlag);	% % % size: (nFs, nTs, nvxs, 3);
		for ic = 1 : 3
			[r.cl{ic}, r.sz{ic}, r.nc{ic}] = theCluster4Power (r.F(:, :, :, ic), nb, th1, th2, nblimit);
		end
		save(fname, 'r');
	...
	%	for ts = 1 : nTestPerCluster
			tic;
			% % % prepare data		
			[F, p] = rmANOVA4ThisData(data, 1, multiCoreFlag);	% % % size: (nFs, nTs, nvxs, 3);

			for ic = 1 : 3
				% % % do cluster
				[tmp1, sz0, tmp3] = theCluster4Power (F(:, :, :, ic), nb, th1, th2, nblimit);
				tmp1 = [];
				tmp3 = [];
	...
	for ic = 1 : 3
		% % % get the thresh
		[r.th(ic), H] = mat_distr2thresh(r.H0(:, :, ic), pth);
...
function [F, p] = rmANOVA4ThisData(data, rFlag, multiCoreFlag)
