function f = plt_icaFigureTemplate(fp, hmv)
...
% % % 	'PaperOrientation', fp.fOri, 
	...
	f.txAX2 = axes('position', fp.txPos2, 'Color', fp.txColorAX, 'parent', f.fid);
	...
	f.tx2 = text(fp.txStartX, fp.txStartY, fp.txS2, 'parent', f.txAX2, ...
		'fontsize', fp.txSZ, 'fontname', fp.txFT, 'fontweight', 'bold', 'Interpreter', 'none' );
	set(f.txAX2, 'box', 'off', 'XTick', [],'YTick', [], 'visible', 'off');
...
% 	set(f.pwAX, 'box', 'off', 'XTick', [],'YTick', [], 'visible', 'off');
	...
	if nargin > 1
		hold on;
		YYY = repmat(1 : size(hmv, 1), [size(hmv, 2), 1])';
		plot(hmv, YYY, '-', 'linewidth', 0.1, 'parent', f.trAX);
	end
end %end of function 
