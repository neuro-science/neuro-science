%% function #1
function c = meg_clusterCoupling2(data, nb, ...
	th1, th2, nClustersPerTest, nTestPerCluster, method, ...
	oid, npools, fileName, nTs, nFs, nVs, th3, andor)
	...
	if nargin < 2 || isempty(data) || isempty(nb)
		...
		[nfs, nts, nvs1, nvs2, nSubs, tmp] = size(data);
		if nvs1 == nvs2 && tmp == 2
			nvs = nvs1;
			clear nvs1 nvs2;
		else
	...
	[myClusters1, sz1, nc1] = cluster4Coupling (con, nb, clp.th1t, clp.th2, clp.method, clp.andor);
	% % % work on negative change
	[myClusters2, sz2, nc2] = cluster4Coupling (-con, nb, clp.th1t, clp.th2, clp.method, clp.andor);
	...
	if clp.npools > 1 % multiple CPU available
		...
		parfor ts = 1 : clp.nTestPerCluster
			...
			% % % do cluster
			[tmp1, sz0, tmp2] = cluster4Coupling (con_r, nb, clp.th1t, clp.th2, clp.method, clp.andor);
			tmp1 = [];
			tmp2 = [];
...
%% function #2
function [myClusters, sz, nc] = cluster4Coupling (conArray, neighborDefinition, conThresh, spfThresh, method, andor)
...
%   14/11/2017: updated by wp for better compatibility
	...
	binArray1 = conArray(:, :, :, :, :, 1) > conThresh;
	binArray2 = conArray(:, :, :, :, :, 2) > conThresh;
	if andor
		binArray = binArray1 & binArray2;
	else
		binArray = binArray1 | binArray2;
	end
	clear binArray1 binArray2;
% 	fprintf('1st floor!\n');	%###
	% % % spatial filtering
