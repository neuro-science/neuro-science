...
function out = meg_coupling_TF (data, trls, methods, flag1d, flagIndTrl)
% % % rewrote 22/11/22, different trials in each cell
% % % rewrote 13/05/14, flag1d added to allow 2d output
	...
	% % % check input
	if nargin < 5
		flagIndTrl = 0;	%default same trials
	end
	...
	if flag1d
		out.size = [out.nPairs, out.nTs, out.nFs];
	else
	...
	for iq = 1 : out.nFs
		for it = 1 : out.nTs
			...
			if flagIndTrl
				nTrlsHere = size(data{it, iq}, 2);
				N = nTrlsHere * nTapers;
				d = reshape(data{it, iq}, [out.nChs, N]);
			else
				N = out.nTrls * nTapers;
				d = reshape(data{it, iq}(:, trls, :), [out.nChs, N]);
			end
