...
% % % updated 13/10/23 by wp: modify to ease the parameter check and special frequency
...
% % % written 20/11/16 by wp: this function is to semi-automatic ICA component rejection
	...
	%% 1. preparison
	% % % some paras
	varString = {'icaWaveDispRange', 'skip_id', 'icaPowerMark', 'freqReductFlag', ...
		'zReductScale'};
	varDefault = {[-1200 3600], [], [2 4 40], true, 1};
	if nargin < 3 || isempty(para)
		for is = 1 : numel(varString)
			cmd = [varString{is}, ' = varDefault{is};'];
		end
	else
		for is = 1 : numel(varString)
			if isfield(para, varString{is})	
				cmd = [varString{is}, ' = para.', varString{is}, ';'];
			else
				cmd = [varString{is}, ' = varDefault{is};'];
			end
			eval(cmd);
	...
	for is = v.cf : v.nFs
		...
		% % % load ica data
		sz = size(fn, 'ica');
		if abs(sz(1) - 1) < 0.1 && sz(2) > 1
			tmp = fn.ica(1, is);
			fprintf('The data was loaded from row vector!');
		elseif abs(sz(2) - 1) < 0.1 && sz(1) > 1
			tmp = fn.ica(is, 1);
			fprintf('The data was loaded from column vector!');
		else
		...
		myIn = {'fColor', [1 1 1], 'cvRangeX', icaWaveDispRange, 'pwMark', icaPowerMark, ...
...
% 	set(f.tpAX, 'box', 'off', 'XTick', [],'YTick', [], 'visible', 'off');
	...
	if ~isempty(fp.pwMark)
		hold(f.pwAX, 'on');
		f.pwy = plot([fp.pwMark(1) fp.pwMark(1)], fp.pwRangeY([1 end]), ...
			'k-', 'linewidth', 0.1, 'parent', f.pwAX);
		f.pwx = plot([fp.pwMark(2) fp.pwMark(3)], fp.pwRangeY([end end]), ...
			'k-', 'linewidth', 0.1, 'parent', f.pwAX);
	end
	set(f.pwAX, 'box', 'off');
...
% 	set(f.pwAX, 'box', 'off', 'XTick', [],'YTick', [], 'visible', 'off');
	...
	NNN = 9;
	for kk = NNN : -1 : 1
		f.tr2(kk) = plot(fp.trx2(:, kk), fp.try, 'k.', 'parent', f.trAX);
	end
end %end of function 
...
% % % written 03/02/17 by wp: this function is to ease plt21_checkICA_matfile
	...
	if isfield (ic, 'lcc')
		str = sprintf(['<',str '> ', 'r_mHM = %4.2f, r_MHM = %4.2f,', ...,
			' r_VEOG = %4.2f, r_HEOG = %4.2f, r_REOG = %4.2f, r_EKG = %4.2f\t'], ...
			nanmean(ic.lcc(ii, :)) * 100, nanmax(ic.lcc(ii, :)) * 100, ...
			ic.acc(ii, [1 2 4 3]) * 10);
	else
		str = sprintf(['<',str '> ', ...,
			' r_VEOG = %4.2f, r_HEOG = %4.2f, r_REOG = %4.2f, r_EKG = %4.2f\t'], ...
			ic.acc(ii, [1 2 4 3]) * 10);
	...
	if isfield(ic, 'fM')
		set(ff.pwAX, 'XLim', [0 ic.fM], 'XTick', [2 4 10 30 50 ic.fM], ...
			'XTickLabel', num2str([2 4 10 30 50 ic.fM]'));
	end
	if ~isempty(fp.pwMark)
		set(ff.pwx, 'YData', [max(ic.yfs(:, ii)) max(ic.yfs(:, ii))]);
		set(ff.pwy, 'YData', [0 max(ic.yfs(:, ii))]);
	end
	...
	if isfield(ic, 'trl')
		for kk = NNN : -1 : 1
				set(ff.tr2(kk), 'XData', ic.trl(:, kk), 'YData', 1 : ic.nm);
		end
	else
		for kk = NNN : -1 : 1
				set(ff.tr2(kk), 'XData', [], 'YData', []);
		end
	end
	set(ff.trAX, 'XLim', [0 1], 'XTIck', [0 0.5 1], ...
		'YLim', [0 ic.nm + 1], 'YTIck', [0 ic.nm + 1]);
	drawnow;
