%% function #1
function c = meg_clusterCouplingANOVA(data, nb, ...
	...
	if nargin < 2 || isempty(data) || isempty(nb)
		...
		% % % check data size [lf1, lf2, nSubs, nfs, nts, nvs1, nvs2];
		[lf1, lf2, nSubs, nfs, nts, nvs1, nvs2] = size(data);
		if nvs1 == nvs2
			nvs = nvs1;
			clear nvs1 nvs2;
		else
	...
	%% 02. data preparison
	if numel(clp.th1) == 1
		clp.th1 = repmat(clp.th1, [3 1]);
	elseif numel(clp.th1) ~= 3
	...
	if max(clp.th1) > 1
		...
		for ic = 3 : -1 : 1
			dn = (nSubs - 1) * nn{ic};
			clp.th1F(ic) = mat_FThreshFromP(nn{ic}, dn, clp.th1(ic));
			fprintf('I compute F threshold %s from P-values: th = %7.3f, as p = %7.5f, N = %d.\n', ...
				cstr{ic}, clp.th1F(ic), clp.th1(ic), nSubs);
			clear dn;
		end
		clear nn;
	end
	% % % initialize H0
	H0 = zeros(clp.nClustersPerTest, 3, clp.nTestPerCluster);
	...
	if nSubs < 12.5
		...
		rng('shuffle');
		s0 = uint32(randperm(2.^nSubs));
		s2 = ismember(dec2bin(s0(1 : nTestPerCluster) - 1, nSubs), '1')';
		clear s0;
	else
		rng('shuffle');
		s1 = rand(nSubs, nTestPerCluster) > 0.5;
		rng('shuffle');
		s2 = rand(nSubs, nTestPerCluster) > 0.5;
	end
	...
	% % % connection real F-value
	fprintf(oid, '\n=========%s: Data computed in on %04d-%02d-%02d %02d:%02d:%02d=========\n', clp.method, round(clock));
	clFlag = true(3, 1);
	F = mat_rmANOVA2Array(data);	
	for ic = 3 : -1 : 1
		[cl{ic}, sz{ic}, nc{ic}] = cluster4Coupling (F(:, :, :, :, ic), nb, clp.th1F(ic), clp.th2, clp.method);
		if isempty(cl{ic})
			clFlag(ic) = false;
			fprintf('\nEmpty here %s!\n', cstr{ic});
		else
	...
	save(clp.fileName, 'cl', 'sz', 'nc', 'F', '-append');
	...
	if clp.npools > 1 % multiple CPU available
		...
		parfor ts = 1 : clp.nTestPerCluster
			...
			d(:, :, s1(:, ts), :, :, :, :) = d([2 1], :, s1(:, ts), :, :, :, :);
			d(:, :, s2(:, ts), :, :, :, :) = d(:, [2 1], s2(:, ts), :, :, :, :);
			tF = mat_rmANOVA2Array(d);	%	[f, t, ch, ch, 3]
			d = [];
			fprintf(oid, 'Data #%03d done after %7.2f seconds.\n', ts, toc);
			szn = zeros(clp.nClustersPerTest, 3);
			for ic = 3 : -1 : 1
				if clFlag(ic)
					tic;
					[tmp1, sz0, tmp2] = cluster4Coupling (tF(:, :, :, :, ic), nb, clp.th1F(ic), clp.th2, clp.method);
					tmp1 = [];
					tmp2 = [];
					...
					if ssz >= clp.nClustersPerTest
						szn(:, ic) = sz0(1 : clp.nClustersPerTest);
						fprintf(oid, 'Hx[%d] = %10.1f.\tTest #%03d done after %7.2f seconds.\n', ic, sz0(1), ts, toc);
					elseif ssz > 0
						szn(1 : ssz, ic) = sz0;
						fprintf(oid, 'Hx[%d] = %10.1f.\tTest #%03d done after %7.2f seconds.\n', ic, sz0(1), ts, toc);
					else
						fprintf(oid, 'Hx[%d] = NaN.\tTest #%03d done after %7.2f seconds.\n', ic, ts, toc);
					end
				end
			end			
			% % % fill H0
	...
	clear ts szn ssz tmp1 tmp2 s1 data;
...
function tfd = cluster_TF2(ed, nFs, nTs)
	...
	for it = 1 : nTs
		for iq = 1 : nFs
			tfd(iq, it) = numel(find(ed(:, 3) == iq & ed(:, 4) == it)); 
		end
	end
end %end of function
