...
% 				kCounter = kCounter + 1;
% 			case 'cvmfile'
% 				p.cvmfile = varargin{kCounter + 1};
% 				kCounter = kCounter + 1;
			case 'ssp'
				p.subsample = varargin{kCounter + 1};
				kCounter = kCounter + 1;
			case 'itm'
	...
	[tri, vc1] = reducepatch(tri, vc, p.subsample);	
	...
	if ~isempty(cv)
		p.crxColor = cv(vcID) * cortex_light + (1-cv(vcID)) * cortex_dark;
	else
...
% 			p.markerSize, p.srcColor, '.', 'parent', p.ax);
	...
	lighting gouraud;
	view(p.viewDir);
	set(p.ax, 'DataAspectRatio', [1 1 1], 'box', 'off');
% 	set(p.ax, 'DataAspectRatio', [1 1 1], 'visible', 'off');
