...
%   17/09/2017: first draft, adopted from the previous function of coupling cluster
	...
	t = mat_tValue4Arrays(conArray(:, :, :, :, 1) - conArray(:, :, :, :, 2), 4, 0);
	[r.c{1}, r.nc1, r.sz1] = insFindCluster(t, tCon, neighborDefinition, pp.spfThresh, pp.method, fShift);
	[r.c{2}, r.nc2, r.sz2] = insFindCluster(-t, tCon, neighborDefinition, pp.spfThresh, pp.method, fShift);
	save(fname, 'r', 'pp', 't');
	...
	if nPools > 1
		...
		parfor ts = 1 : pp.nTestPerCluster
			...
			% % % do clustering
			[tmp1, tmp2, sz0] = insFindCluster(t1, tCon, neighborDefinition, pp.spfThresh, pp.method, fShift);
			tmp1 = [];
			tmp2 = [];
			cz = [];
			t1 = [];
