...
function [res1, res2]=cs2nodes_mim_clean(cs,F1,F2)

% % % It is guido's function, here just remove the switches and outputs to
% % % speed up
...
%     if round(i/1000)*1000==i;disp(i);end
    ...
    caainvsqrt=sqrtm(csvoxrealinv(:,:,i));
	 ...
	 resloc1 = zeros(ng2, 1); %initialize
	 resloc2 = zeros(ng2, 1); %initialize
    for j=1:ng2;
%         if nargin==2;
%             Floc2=squeeze(F1(:,j,:));
%         else
